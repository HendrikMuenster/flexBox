...
//L1TVOpticalFlow(image1,image2,tol,lambda,maxIterations,norm,inputV,inputY,stepsize,discretization,numberOfWarps)
{
	...
	while (entry < nrhs-2)
	{
		...
		if (strcmp(func_type, "primal") == 0)
		{
			printf("%s\n", class_name);
			
			//all primal term have alpha and dims as first arguments

			//first argument is factor:
			float alpha = (float)mxGetScalar(prhs[entry + 2]);
			//second is an array of dimensions
			flexVector<int> _dims;
			double *input_dims = mxGetPr(prhs[entry + 3]);
			...
			if (strcmp(class_name, "L2dataTerm") == 0)
			{
				...
				mainObject.addPrimal(new flexL2DataTerm<float>(_dims, alpha, f));
			...
			else if (strcmp(class_name, "L1dataTerm") == 0)
			{
				...
				//append primal term to list of primal terms
				mainObject.addPrimal(new flexL1DataTerm<float>(_dims, alpha, f));
			...
			{
				//append primal term to list of primal terms
				mainObject.addPrimal(new flexEmptyDataTerm<float>(_dims));
		...
		else if (strcmp(func_type, "dual") == 0)
		{
			...
			{
				mainObject.addDual(new flexL1dualizedOperatorIso<float>(alpha, operatorList), corrPrimal);
				entry += 5;
			}
			else if (strcmp(class_name, "L1dualizedOperatorAniso") == 0)
			{
				mainObject.addDual(new flexL1dualizedOperatorAniso<float>(alpha, operatorList), corrPrimal);
				entry += 5;
			}
			else if (strcmp(class_name, "L2dualizedOperator") == 0)
			{
				mainObject.addDual(new flexL2dualizedOperator<float>(alpha, operatorList), corrPrimal);
				entry += 5;
			}
			else if (strcmp(class_name, "L1dualizedDataTerm") == 0)
			{
				//6th is data array
			...
			//NStructElems = mxGetNumberOfElements(prhs[entry + 3]]);
	...
	const mwSize resultSize[2] = { mainObject.getDims(0)[0], mainObject.getDims(0)[1] };
	...
	for (int i = 0; i < resultSize[0] * resultSize[1]; ++i)
	{
